...
Scenario: I can create a booking
  ...
  And select "United States" from "customer_profile_billing[commerce_customer_address][und][0][country]"
  ...
  Then I click on "Edit" on the row containing "test_package_all"
  And I fill in "rooms_package_dates[und][0][value][date]" with ""
  And I fill in "rooms_package_dates[und][0][value][date]" with "04/05/2015"
  And I fill in "rooms_package_dates[und][0][value2][date]" with ""
  And I fill in "rooms_package_dates[und][0][value2][date]" with "04/15/2015"
  And I press the "Save product" button
